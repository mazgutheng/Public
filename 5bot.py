...
staff=["ucad303333969352466bfecd62089a1b4"]
...
wait = {
    ...
    'timeline':True,
    "Timeline":True,
    "comment1":"Auto Like By alien x\nline.me/ti/p/%40qic0604r",
    "comment2":"Auto Like By alien x\nline.me/ti/p/%40qic0604r",
    "comment3":"Auto Like By alien x\nline.me/ti/p/%40qic0604r",
    "comment4":"Auto Like By alien x\nline.me/ti/p/%40qic0604r",
    "comment5":"Auto Like By alien x\nline.me/ti/p/%40qic0604r",    
    "commentOn":True,
    "commentBlack":{},
    ...
    try:
	 ...
	 if wait["Ghost"] == True:
		...
		cl.sendText(msg.to,"Itu Owner Kami")
                ...
                msg.contentMetadata = {'mid': pembuat}
                random.choice(KAC).sendMessage(msg)               
		...
		cl.sendText(msg.to,"Itu Yang Buat Grup Ini")
            ...
            elif msg.text in ["Creator menu","creator menu","Creator Menu"]:
		...
		gid = cl.getGroupIdsJoined()
            ...
            elif msg.text in ["Buka qr","Open qr","Buka","buka","Open"]:
		 if op.param3 in Creator:
 		  if op.param3 in admin:
            ...
            elif msg.text in ["Tutup qr","Close qr","Tutup","tutup","Close"]:
		 if op.param3 in Creator:
 		  if op.param3 in admin:
 	 	   if op.param3 in staff:
			if msg.toType == 2:
...
#		    cl.sendText(msg.to,"Khusus Admin")
		...
		if msg.from_ in admin:
	     ...
	     if op.param3 in Creator:
 	      if op.param3 in admin:
            ...
            elif msg.text in ["My set","My set"]:
                md = ""
		...
		if wait["Simi"] == True: md+="║ ✔️ Simisimi ~ On\n"
            ...
            elif msg.text in ["Alien1 Gift","Alien1 gift"]:
                msg.contentType = 9
            ...
            elif msg.text in ["Alien2 Gift","Alien2 gift"]:
                msg.contentType = 9
            ...
            elif msg.text in ["Alien3 Gift","Alien3 gift"]:
                msg.contentType = 9
		...
		try:
            ...
            elif msg.text in ["Keluar","keluar"]:
              if wait["Leave"] == True:		    
...
#		          cl.sendText(msg.to,"Bilang Dulu Sama Admin Ku")		    
		...
		cl.sendText(msg.to,"αlíҽղ|ᵖʳᵒᵗᵉᶜᵗ")
                ki.sendText(msg.to,"αlíҽղ1|ᵖʳᵒᵗᵉᶜᵗ")
                kk.sendText(msg.to,"αlíҽղ2|ᵖʳᵒᵗᵉᶜᵗ")
                kc.sendText(msg.to,"αlíҽղ3|ᵖʳᵒᵗᵉᶜᵗ")
                kr.sendText(msg.to,"αlíҽղ4|ᵖʳᵒᵗᵉᶜᵗ")
		...
		if msg.from_ in Creator:
		    ...
		    cl.sendText(msg.to,"Lu sape!")
            ...
            elif "Hitam @" in msg.text:
                if msg.from_ in admin:
                  if msg.toType == 2:
                    print "@Hitam by mention"
              ...
              if msg.from_ in admin:
                ...
                if msg.from_ in admin:
                    _name = msg.text.replace("Hitam remove @","")
                    _nametarget = _name.rstrip('  ')
                    gs = ki.getGroup(msg.to)
                    gs = kk.getGroup(msg.to)
                    gs = kc.getGroup(msg.to)
...
#                cl.sendText(msg.to,"☆Ketik ☞Help☜ Untuk Bantuan☆") 
		...
		if msg.from_ in Creator:
	        ...
	        if msg.from_ in Creator:                
              ...
              if msg.from_ in Creator:
                msg.contentType = 13
                msg.contentMetadata = {'mid': "ALIEN,'"}
                cl.sendMessage(msg)

 
            elif "Alien copy @" in msg.text:
              if msg.from_ in Creator:
                   print "[COPY] Ok"
                   _name = msg.text.replace("Alien copy @","")
                   _nametarget = _name.rstrip('  ')
                   gs = cl.getGroup(msg.to)
                   targets = []
                   for g in gs.members:
            ...
            elif "Alien1 copy @" in msg.text:
              if msg.from_ in Creator:
                   print "[COPY] Ok"
                   _name = msg.text.replace("Alien1 copy @","")
                   _nametarget = _name.rstrip('  ')
                   gs = ki.getGroup(msg.to)
                   targets = []
                   for g in gs.members:
                       ...
                       for target in targets:
                            try:
                               ...
                               ki.sendText(msg.to, "Copied (^_^)")
                                ...
                                print e
            elif "Alien2 copy @" in msg.text:
              if msg.from_ in Creator:
                   print "[COPY] Ok"
                   _name = msg.text.replace("Alien2 copy @","")
                   _nametarget = _name.rstrip('  ')
                   gs = kk.getGroup(msg.to)
                   targets = []
                   for g in gs.members:
            ...
            elif "Alien3 copy @" in msg.text:
		if msg.from_ in Creator:
                   print "[COPY] Ok"
                   _name = msg.text.replace("Alien3 copy @","")
                   _nametarget = _name.rstrip('  ')
                   gs = kc.getGroup(msg.to)
                   targets = []
                   for g in gs.members:
            ...
            elif "Alien4 copy @" in msg.text:
		if msg.from_ in Creator:
                   print "[COPY] Ok"
                   _name = msg.text.replace("Alien4 copy @","")
                   _nametarget = _name.rstrip('  ')
                   gs = kr.getGroup(msg.to)
                   targets = []
                   for g in gs.members:
	     ...
	     if op.param3 in Creator:
 	      if op.param3 in admin:
            ...
            elif "/cnalien" in msg.text:
		if msg.from_ in Creator:
                    string = msg.text.replace("/cnalien","αlíҽղ|ᵖʳᵒᵗᵉᶜᵗ")
            ...
            elif "/cnalien1" in msg.text:
		if msg.from_ in Creator:
                    string = msg.text.replace("/cnalien1","αlíҽղ1|ᵖʳᵒᵗᵉᶜᵗ")
                    if len(string.decode('utf-8')) <= 5000:
            ...
            elif "/cnalien2" in msg.text:
		if msg.from_ in Creator:
                    string = msg.text.replace("/cnalien2","αlíҽղ2|ᵖʳᵒᵗᵉᶜᵗ")
                    if len(string.decode('utf-8')) <= 5000:
            ...
            elif "/cnalien3" in msg.text:
		if msg.from_ in Creator:
                    string = msg.text.replace("/cnalien3","αlíҽղ3|ᵖʳᵒᵗᵉᶜᵗ")
                    if len(string.decode('utf-8')) <= 5000:
            ...
            elif "/cnalien4" in msg.text:
		if msg.from_ in Creator:
                    string = msg.text.replace("/cnalien4","αlíҽղ4|ᵖʳᵒᵗᵉᶜᵗ")
                    if len(string.decode('utf-8')) <= 5000:
                        profile = cl.getProfile()
                        profile.displayName = string
                        kr.updateProfile(profile)
	     ...
	     if op.param3 in Creator:
 	      if op.param3 in admin:
            ...
            elif msg.text in ["Alien acc invite"]:
                if msg.from_ in admin:
                    gid = cl.getGroupIdsInvited()
                    _list = ""
                    for i in gid:
            ...
            elif msg.text in ["Alien1 acc invite"]:
                if msg.from_ in admin:
                    gid = ki.getGroupIdsInvited()
                    _list = ""
                    for i in gid:
            ...
            elif msg.text in ["Alien2 acc invite"]:
                if msg.from_ in admin:
                    gid = kk.getGroupIdsInvited()
                    _list = ""
                    for i in gid:
            ...
            elif msg.text in ["Alien3 acc invite"]:
                if msg.from_ in admin:
                    gid = kc.getGroupIdsInvited()
                    _list = ""
                    for i in gid:
            ...
            elif msg.text in ["Alien4 acc invite"]:
                if msg.from_ in admin:
